...
# Optional, generated by scripts/setup_test_backends.py. Keep model paths and
# isolated interpreters in a local ignored file instead of the user's shell.
TEST_ENV_FILE="${MHCTOOLS_TEST_ENV:-$(dirname "$0")/env/test-backends/activate.sh}"
if [[ -n "${MHCTOOLS_TEST_ENV:-}" || -f "$TEST_ENV_FILE" ]]; then
